...
import baselines.ensemble_ddpg.training as training
from baselines.ensemble_ddpg.models import Actor, Critic, AHEActor, AHECritic
from baselines.ensemble_ddpg.memory import Memory
from baselines.ensemble_ddpg.noise import *
...
def run(env_id, seed, noise_type, layer_norm, evaluation, num_critics, **kwargs):
    ...
    training.train(env=env, eval_env=eval_env, param_noise=param_noise,
        action_noise=action_noise, actor=actor, critics=critics, memory=memory, **kwargs)
    env.close()
    if eval_env is not None:
        eval_env.close()
    if rank == 0:
...
def parse_args():
    ...
    print("[Tiancheng] Now we're using ensemble DDPG")
    ...
    parser.add_argument('--critic-l2-reg', type=float, default=0.0)
