...
def run_experiment_lite(
  ...
  **kwargs):
    ...
    for task in batch_tasks:
        ...
        # use pickle(cloudpickle) to compress 'stub_method_call'
        #   encode with base64 and decode in utf-8 format
        if use_cloudpickle:
            import cloudpickle
        ...
        # configure basic task setting
        exp_count += 1
        # TODO: not used variable "params"
    ...
    # TODO: add support to remote experiment calls
    if mode == "local":
        for task in batch_tasks:
            env = task.pop("env", None)

            # generate local command
            # config.PROJECT_PATH: the container path for source codes
            # script: the exact runner ( script ) for launching experiments
            ...
            if print_command:
                print(command)
            if dry:
                # TODO: explain the meaning of dry ?
                return
            try:
                if env is None:
                    env = dict()
                if wait_subprocess:
                    # wait flag will force the whole process wait until subprocess finished
                    subprocess.call(
                        command, shell=True, env=dict(os.environ, **env))
                    popen_obj = None
                else:
                    # start and detach a sub process for launching experiments
